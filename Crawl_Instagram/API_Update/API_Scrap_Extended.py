...
def scrape_user_fallback(username: str):
    try:
        ...
        print(f"[Fallback] httpx bị chặn hoặc lỗi, thử requests JSON API...")
    
    try:
        return scrape_user_requests(username)
    except Exception:
        print(f"[Fallback] requests JSON API bị chặn, thử requests parse HTML...")
    
    try:
        return scrape_user_html_requests(username)
    except Exception:
        print(f"[Fallback] requests parse HTML bị chặn, thử Selenium...")

    try:
        return scrape_user_selenium(username)
    except Exception:
        print(f"[Fallback] Selenium cũng bị chặn.")
        raise 

current_dir = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(current_dir, "instagram_data.json")

# Thay đổi: Sử dụng file instagram.txt nằm cùng thư mục với script thay vì đường dẫn cứng
file_input_path = os.path.join(current_dir, "urls.txt")
...
for username in usernames:
    try:
        ...
        save_to_file(file_path, parsed_data) 
